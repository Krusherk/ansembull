...
export default function GameOverScreen({
  ...
  const [submitStatus, setSubmitStatus] = useState<'submitting' | 'submitted' | 'error'>('submitting');
  ...
  useEffect(() => {
    if (!walletAddress) {
      ...
      setSubmitResult('No wallet connected');
      return;
    }
    ...
    const autoSubmit = async () => {
      ...
      if (result.success) {
        ...
        setSubmitResult(result.rank ? `Rank #${result.rank}` : 'Score saved!');
      } else {
        setSubmitStatus('error');
        setSubmitResult(result.error || 'Failed to save');
      }
    };
  ...
  return (
    <div className="screen-overlay gameover-screen">
      <div className="gameover-content">
        ...
        {/* Auto-submit status */}
        <div className={`gameover-submit-status status-${submitStatus}`}>
          {submitStatus === 'submitting' && (
            <><span className="spinner" /> Saving score...</>
          )}
